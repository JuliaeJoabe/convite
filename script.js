function countDown() {
  const diasCasamento = new Date('16 November 2024 17:00:00');
...
function sugerirMusica(event) {
  if (input.value.length > 0 || sugerir.value.length > 0) {
    const link = `https://api.whatsapp.com/send/?phone=555391544559&text=Olá,+gostaria+de+sugerir+a+música+para+a+festa :+${
      input.value
